...
const COOLDOWN_SECS: u64 = 60; // 1 min cooldown — longtail arbs are dynamic
...
/// Binary search iterations for optimal 2-hop amount
const BINARY_SEARCH_ITERS_2HOP: u32 = 10;
/// Min arb input (0.01 ETH)
const MIN_ARB_WEI: f64 = 1_000_000_000_000_000.0; // 0.001 ETH (was 0.01)
...
pub fn detect_arb(
    ...
    // SPEED > OPTIMAL: return FIRST profitable arb (every ms counts in MEV)
    for i in 0..relevant_pools.len() {
        for j in (i + 1)..relevant_pools.len() {
            ...
            // HIGH-LATENCY FILTER: skip UniV3↔UniV3 with SAME fee tier
            // Those are captured by co-located bots. But different fee tiers
            // (e.g. 0.05% vs 1%) create persistent cross-pool arbs we CAN capture.
            if is_high_competition(pool_a) && is_high_competition(pool_b)
                && pool_a.fee_bps == pool_b.fee_bps
            {
                continue;
            }
...
fn check_pair_arb(
    ...
    // ── Binary search for optimal amount (maximizes profit) ──
    // Tests multiple input sizes to find the sweet spot where price impact
    // doesn't eat all the spread. Much better than formula-based sizing.

    let v2_cap = get_v2_reserve_cap(buy_pool, sell_pool, &swap.token_in);
    let max_amount = v2_cap.unwrap_or(MAX_ARB_WEI).min(MAX_ARB_WEI);

    let mut best_profit_f = 0.0_f64;
    let mut best_amount = U256::ZERO;
    let mut best_bought = U256::ZERO;
    let mut best_sold = U256::ZERO;

    let mut lo = MIN_ARB_WEI;
    let mut hi = max_amount;

    for _ in 0..BINARY_SEARCH_ITERS_2HOP {
        let mid = (lo + hi) / 2.0;
        let test_in = U256::from(mid as u128);

        let profit_f = simulate_swap(buy_pool, &swap.token_in, &test_in)
            .and_then(|bought| {
                simulate_swap(sell_pool, &swap.token_out, &bought).map(|sold| (bought, sold))
            })
            .map(|(bought, sold)| {
                if sold > test_in {
                    let p = u256_to_f64(sold - test_in) / 1e18;
                    if p > best_profit_f {
                        best_profit_f = p;
                        best_amount = test_in;
                        best_bought = bought;
        ...
        // Binary search: if profit is increasing, try larger; else try smaller
        if profit_f > 0.0 {
            lo = mid; // profit positive, try bigger
        } else {
            hi = mid; // no profit, try smaller
        }
    }

    let optimal_in = best_amount;
    let sim_bought = best_bought;
    let sim_sold = best_sold;
    let profit_eth = best_profit_f;
    let profit = if best_sold > optimal_in { best_sold - optimal_in } else { U256::ZERO };

    if profit_eth == 0.0 || optimal_in.is_zero() {
        if let Some(log) = logger {
            log.log_spread(
    ...
    // Gas on Arbitrum: 350k gas × 0.01 gwei ≈ 0.0000035 ETH + L1 posting ~0.00001
    // Actual observed: ~0.00001 ETH for successful arbs on Arbitrum
    let gas_cost_eth = 0.00002;
...
fn simulate_v3_swap(pool: &Pool, token_in: &Address, amount_in: &U256) -> Option<U256> {
    ...
    // 15% safety margin for multi-tick crossing (single-tick sim overestimates)
    // 30% was too conservative — rejecting viable arbs. On-chain sim is the real gate.
    let amount_out_safe = amount_out * 0.85;
    Some(U256::from(amount_out_safe as u128))
}

/// Check if a pool has enough liquidity and fresh-enough reserves for arb
fn is_pool_viable(pool: &Pool) -> bool {
    if pool.fee_bps == 0 { return false; }
    // Reject pools with reserves older than 30 seconds
    if let Some(last_update) = pool.last_update {
        if last_update.elapsed().as_secs() > 30 {
            return false; // stale reserves
        }
    } else {
        // Never updated — only allow if reserves look non-zero (hardcoded pools)
        if pool.reserve0.is_zero() && pool.sqrt_price_x96.is_none() {
            return false;
        }
    }
    if is_v2(pool.dex) || is_curve(pool.dex) {
...
fn simulate_multi_hop(hops: &[(& Pool, Address, Address)], amount_in_f: f64) -> Option<f64> {
    ...
    for (_i, (pool, token_in, _token_out)) in hops.iter().enumerate() {
        // On-chain _doMultiHop uses full balance (no buffer) — match it exactly
...
fn find_optimal_multihop_amount(hops: &[(&Pool, Address, Address)]) -> Option<(f64, f64)> {
    ...
    for _ in 0..12 { // ~12 iterations = precision to ~0.001 ETH (fast enough for MEV)
        let mid = (lo + hi) / 2.0;
    ...
    for p_ab in &pools_ab {
        ...
        for p_bc in &pools_bc {
            ...
            for p_ca in &pools_ca {
                ...
                let gas_cost = 0.000015; // 3 swaps + flash loan (Arbitrum low gas)
...
async fn try_4hop_cycle(
    ...
    let est_profit = profit_raw / 1e18;
    let gas_cost = 0.000020; // 4 swaps + flash loan (Arbitrum low gas)
    let net_profit = est_profit - gas_cost;
