...
pub struct Pool {
    ...
    /// When reserves/state were last updated (for staleness detection)
...
impl PoolState {
    ...
    pub async fn update_reserves(&self, pool_addr: Address, reserve0: U256, reserve1: U256) {
        if let Some(pool) = self.pools.write().await.get_mut(&pool_addr) {
            ...
            pool.last_update = Some(Instant::now());
        }
    }
    ...
    /// Get all pools for a given token pair (single lock, minimal cloning)
    pub async fn get_pools_for_pair(&self, token_a: Address, token_b: Address) -> Vec<Pool> {
        let pair_key = PairKey::new(token_a, token_b);
        // Single lock: read pair_pools first (cheap), then pools
        let addrs = {
            let pair_pools = self.pair_pools.read().await;
            match pair_pools.get(&pair_key) {
                Some(a) => a.clone(),
                None => return Vec::new(),
            }
        };
        // Now take pools lock separately (no nested locks = no contention)
        let pools = self.pools.read().await;
        addrs.iter().filter_map(|addr| pools.get(addr).cloned()).collect()
    }
