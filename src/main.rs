...
async fn main() -> Result<()> {
    ...
    let rpc_url = wait_for_rpc(&config.node_rpc_url).await?;
    ...
    // Use public RPC that supports eth_chainId (needed by alloy for signing)
    // The sequencer endpoint only accepts eth_sendRawTransaction
    let send_rpc = "https://arb1.arbitrum.io/rpc";
    let send_provider = ProviderBuilder::new()
        .wallet(eth_wallet)
        .connect_http(send_rpc.parse()?);
    info!(rpc = send_rpc, "Send provider (sequencer direct)");

    let chain_id = node_provider.get_chain_id().await?;
    info!(chain_id, "Connected");

    let block = node_provider.get_block_number().await?;
    info!(block, "Block");

    let balance = node_provider.get_balance(config.bot_address).await?;
    ...
    // ─── Background: reserve refresh (V2 fast @ 5s, V3 slower @ config interval) ───
    ...
    // Local simulator via rpc-cache (~5ms per sim)
    ...
    while let Some(swap) = swap_rx.recv().await {
        ...
        // When we see a swap on pool X, we know pool X's price just changed.
        // Other pools for the same pair haven't changed yet → arb opportunity.
        if swap.pool != Address::ZERO {
            if let Some(amount_in_val) = Some(swap.amount_in).filter(|a| !a.is_zero()) {
                // Compute post-swap reserves for V2 pools
                ...
                if let Some(pool) = pools_w.get_mut(&swap.pool) {
                    if pool.reserve0 > alloy::primitives::U256::ZERO && pool.reserve1 > alloy::primitives::U256::ZERO {
                        // V2: update reserves based on swap direction
        ...
        // Arb detection (pure compute) — now with updated post-swap price
...
async fn wait_for_rpc(url: &str) -> Result<String> {
